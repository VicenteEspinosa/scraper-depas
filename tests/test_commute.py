...
def test_any_number_of_locations_can_be_configured(monkeypatch):
    ...
    assert [place.name for place in prefs().locations()] == ["a", "b", "c"]
    assert (prefs().locations()[1].lat,
            prefs().locations()[1].lon) == (-33.4983, -70.6114)
...
def test_no_locations_configured_measures_nothing():
    ...
    assert from_listing(None, *ESTACION_CENTRAL, prefs().locations()) == {}
...
def test_every_location_is_measured(monkeypatch, router):
    ...
    assert from_listing(None, *ESTACION_CENTRAL, prefs().locations()) == {"a": 11, "b": 22}
...
def test_a_routed_answer_beats_the_offline_estimate(monkeypatch, router):
    ...
    assert from_listing(None, *ESTACION_CENTRAL, prefs().locations()) == {"a": 11}
