...
| `DEPAS_AVAILABILITY_TARGET` | The move-in date you are aiming for, `YYYY-MM-DD`. Scored on distance rather than as a deadline: landing on it is full marks, a month either side is 80, and two months is 40 — freeing up early is rent on a flat you cannot use yet, freeing up late leaves you nowhere to live. Never a cutoff, and a listing that stated no date is left unscored on it, because most portals simply do not publish the field. |
