...
| `DEPAS_AVAILABILITY_TARGET` | The move-in date you are aiming for, `YYYY-MM-DD`. Scored on distance rather than as a deadline, and the two sides are not the same: everything that frees up between **today and that date** is in play, worth 80 at the near end of the window and 100 on the date itself, while past the date the score falls a whole span per month. Taking a flat early only costs you the overlap; taking it late leaves you nowhere to live. Never a cutoff, and a listing that stated no date is left unscored on it, because most portals simply do not publish the field. |
