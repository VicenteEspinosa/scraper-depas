...
| Comunas | A paged checklist of the 32 in the Provincia de Santiago, where each press cycles the commune through the three things it can be: **✅** one you want, **👎** one you would take but that loses score for it, **⬜** one nothing even scrapes. The other eleven RM communes the portal indexes are an hour out, so they are typed rather than scrolled past every time — and once chosen, one shows up first in the checklist so it can be cycled like any other. A typed commune joins the ✅ tier, never the 👎 one. |
...
| `DEPAS_COMMUNES` again, as a ranking | Communes in tiers, best first, the same `>` and `,` the metro lines use: `nunoa,providencia > santiago`. Every tier is scraped and alerted on exactly as a flat list always was — the tiers only move the grade, with the first worth 100, the last 40 and anything between spread evenly. A commune in no tier is still never looked at, and scores 0 on the component if a pasted link puts one in front of you anyway. One tier ranks nothing, so the **comuna** component stays off until there are two, and every configuration written before this keeps meaning what it meant. |
